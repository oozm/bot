 ...
 * de se faire rogner.
 */
export const RAYON_MAX = Math.max(
  ...SHAPES.map((forme) => Math.max(...forme.radii)),
)
 ...
 */
export const cycleAccepteTransparence = (format: FormatCycle) =>
  format === 'gif'
...
export const ACTION_BY_ID = new Map<string, ActionExport>(
