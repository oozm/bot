...
describe('cadre d export', () => {
  ...
  it('contient toutes les formes du personnalisateur', () => {
    for (const forme of SHAPES) {
      ...
      expect(rayon, `la forme « ${forme.id} » depasse du cadre`).toBeLessThan(
        DEMI_CADRE,
      )
    }
  })
  ...
  it('produit un viewBox carre centre sur la boule', () => {
    ...
    expect(viewBoxExport()).toBe(
      `${-DEMI_CADRE} ${-DEMI_CADRE} ${DEMI_CADRE * 2} ${DEMI_CADRE * 2}`,
    )
  })
})
...
describe('catalogue des exports', () => {
  ...
  it('ne propose qu un seul png a telecharger', () => {
    const pngs = ACTIONS.filter(
      (a) => a.mode === 'telecharge' && a.extension === 'png',
    )
...
describe('nom de fichier', () => {
  ...
  it('translittere les accents et separe les mots', () => {
    expect(nomFichier('Cycle par défaut', '', '', 'mp4')).toBe(
      'xbot-cycle-par-defaut.mp4',
    )
    expect(nomFichier('Été 2026', '', '', 'gif')).toBe('xbot-ete-2026.gif')
  })

  it('survit a des ids vides', () => {
    expect(nomFichier('', '', '', 'png')).toBe('xbot.png')
  })
})
