...
export function svgAnime(
  ...
  if (parImage !== n)
    throw new Error(`${n} yeux dans le masque, ${parImage} par image cle`)
...
function arrete(palette: PaletteGif) {
  ...
  const tries = [...palette.vues.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 255)
...
export function gifIndexe(
  ...
  for (const cle of couleurs.keys())
    table.push((cle >> 16) & 0xff, (cle >> 8) & 0xff, cle & 0xff)
