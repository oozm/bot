 ...
 */
export function svgAutonome(
  svg: SVGSVGElement,
  taille: number,
  viewBox = viewBoxExport(),
) {
...
export async function versPng(markup: string, taille: number): Promise<Blob> {
  ...
  return await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob(
      (blob) =>
        blob ? resolve(blob) : reject(new Error('encodage png impossible')),
      'image/png',
    )
  })
}
  ...
  try {
    return await versMp4(
      ...
      async (i) => {
        ...
        await dessine(
          svgAutonome(svg, taille, viewBoxExport(DEMI_ECRAN)),
          taille,
          canvas,
          fond,
        )
...
export async function cycleVersGif(
  ...
  const passe = async (
    lis: (index: number, pixels: Uint8ClampedArray) => void,
  ) => {
  ...
  try {
    ...
    return new Blob(
      [gifIndexe(palette, morceaux, taille, taille, Math.round(pas * 1000))],
      {
        type: 'image/gif',
      },
    )
...
function matricesDesYeux(svg: SVGSVGElement) {
  return [...svg.querySelectorAll('mask [transform]')].map((e) =>
