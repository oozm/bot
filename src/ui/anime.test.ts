...
describe('svg anime', () => {
  ...
  it('remplace le transform de chaque oeil par une classe', () => {
    ...
    expect(sortie.match(/<mask[\s\S]*?<\/mask>/)![0]).not.toContain(
      'transform="matrix',
    )
  })
  ...
  it('ecrit une regle de keyframes par oeil, aux bons pourcentages', () => {
    expect(sortie).toContain(
      '@keyframes oeil0{0%{transform:matrix(1,0,0,1,0,0)}',
    )
  ...
  it('refuse ce qu il ne sait pas animer', () => {
    ...
    expect(() =>
      svgAnime(BASE, [['matrix(1,0,0,1,0,0)'], ['matrix(1,0,0,1,1,0)']], 3),
    ).toThrow()
  })
})
...
function litGif(f: Uint8Array) {
  const txt = (o: number, n: number) =>
    String.fromCharCode(...f.subarray(o, o + n))
...
describe('gif anime', () => {
  ...
  it('convertit le delai en centiemes', () => {
    expect(
      litGif(suite.length ? gifAnime(suite, 16, 16, 50) : new Uint8Array())
        .delai,
    ).toBe(5)
