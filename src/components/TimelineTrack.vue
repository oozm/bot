...
watch(scale, (now, before) => {
  ...
  if (!el) return
  const x =
    (anchorX ?? el.getBoundingClientRect().left + el.clientWidth / 2) -
...
onMounted(onScroll)
watch(
  () => [total.value, props.blocks] as const,
  () => nextTick(onScroll),
)
...
watch(block, () => {
  ...
  if (
    x < el.scrollLeft ||
    x + width(block.value) > el.scrollLeft + el.clientWidth
  ) {
...
function removeBlock(index: number) {
  ...
  else if (block.value >= props.blocks.length - 1)
    block.value = props.blocks.length - 2
}
...
function onResizeDown(index: number, e: PointerEvent) {
  ...
  resize.value = {
    index,
    startX: e.clientX,
    startDuration: props.blocks[index]!.duration,
  }
}
...
async function onCardKey(index: number, e: KeyboardEvent) {
  ...
  if (!e.altKey) {
    ...
    emit(
      'seek',
      Math.max(0, Math.min(total.value - 0.001, at.value + sens * STEP)),
    )
    return
  }
      ...
      >
        ...
        <ul class="flex flex-1 items-stretch">
          ...
          <li
            ...
            :class="
              lifted(i) ? 'z-20' : 'transition-transform duration-150 ease-out'
            "
            :style="{
          ...
          >
            ...
            >
              ...
              <span
                ...
                :class="
                  i === block ? 'text-[var(--ink)]' : 'text-[var(--muted)]'
                "
              >
                {{ secondes(b.duration) }}
              </span>
