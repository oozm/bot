...
function onNamed(name: string) {
  ...
  cycles.value = cycles.value.map((c) =>
    c.id === demande.id ? { ...c, name: unique } : c,
  )
}
...
<template>
  ...
  >
    <!-- lecture : flottante au-dessus de la piste, au centre, le temps ecoule a
         ...
         en bas a droite. -->
    <div
      class="absolute -top-5 left-1/2 flex -translate-x-1/2 items-center gap-3"
    >
      <span class="text-sm font-medium tabular-nums max-lg:hidden">
        {{ mmss(at) }}
      </span>
      <button
        type="button"
      ...
      <span class="text-sm tabular-nums text-[var(--muted)] max-lg:hidden">
        {{ mmss(total) }}
      </span>
    </div>
    ...
    <div class="flex h-full flex-col gap-2 select-none">
      ...
      <div class="flex shrink-0 items-center justify-end gap-4 max-sm:gap-2">
        ...
        <p class="text-xs tabular-nums text-[var(--muted)]">
          <span class="text-[var(--ink)]">{{ mmss(at) }}</span>
          / {{ mmss(total) }}
        </p>
    ...
    <NameDialog
      ...
      :title="
        naming?.mode === 'rename'
          ? t('dialog.nameRenameTitle')
          : t('dialog.nameCreateTitle')
      "
    ...
    <ConfirmDialog
      ...
      :title="
        t('dialog.removeTitle', { name: removing ? nomDeCycle(removing) : '' })
      "
